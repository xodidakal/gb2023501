...
<script type="text/javascript">
	function gameList() {
		...
		location.href = "gameList?keyword="+keyword;
	}
...
<div class="row g-0 justify-content-center">
	<div class="col-lg-11 wow fadeInUp" data-wow-delay="0.5s">
		<div class="mb-9">
	         <!-- heading -->
	         <h2 style="margin-bottom: 15px;">게임콘텐츠 관리</h2>
	         <p style="margin-bottom: 35px;">총 <fmt:formatNumber value="${totalSearchGame}" groupingUsed="true"/> 건</p>
	    </div>
	    ...
	    <!-- 게임목록 -->
	        	<table class="listTable">
	        		<thead>
					 ...
					 <c:forEach var="game" items="${listGame}" varStatus="status">
		                ...
		                <nav aria-label="Page navigation example">
						  <ul class="pagination">
						  	<c:if test="${page.startPage > page.pageLimit}">
						  		<li class="page-item"><a class="page-link" href="gameList?currentPage=${page.startPage-page.pageLimit}">이전</a></li>
						  	</c:if>
						    <c:forEach var="i" begin="${page.startPage }" end="${page.endPage }">
						    	<li class="page-item"><a class="page-link" href="gameList?currentPage=${i }">${i }</a></li>
						    </c:forEach>
						 	<c:if test="${page.endPage < page.totalPage}">
						 		<li class="page-item"><a class="page-link" href="gameList?currentPage=${page.startPage+page.pageLimit}">다음</a></li>
						 	</c:if>
						  </ul>
						</nav>
					</div>
